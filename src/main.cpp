...
static const unsigned int CHECKLOCKTIMEVERIFY_SWITCH_TIME = 1518134400; // 09-Fev-18 00:00:00 UTC
...
const string strMessageMagic = "MNcash Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // MNcash: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool AbortNode(const std::string &strMessage, const std::string &userMessage) {
    ...
    userMessage.empty() ? _("Error: A fatal internal error occured, see debug.log for details") : userMessage, "", CClientUIInterface::MSG_ERROR);
    StartShutdown();
    return false;
}
                        ...
                        bool* pfMissingInputs)
{
    ...
    // MNcash: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    int64_t nSubsidy = 10 * COIN; //Inicial
    if(pindexBest->nHeight < 1)
        nSubsidy = 60000 * COIN; //10 MasterNode Initial

    if(pindexBest->nHeight >= 1 && pindexBest->nHeight < 101)
        nSubsidy = 0 * COIN;

    if(pindexBest->nHeight >= 100 && pindexBest->nHeight < 10001)
        nSubsidy = 10 * COIN;

    if(pindexBest->nHeight >= 10000 && pindexBest->nHeight < 200001)
        nSubsidy = 7 * COIN;

    if(pindexBest->nHeight >= 200000 && pindexBest->nHeight < 1000001)
        nSubsidy = 2.5 * COIN;

    if(pindexBest->nHeight >= 1000000)
        nSubsidy = 2 * COIN;

    // LAST_POW_BLOCK = 25001
...
// MNcash: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes MNcash fix for wrong retargeting difficulty by Mammix2
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // MNcash: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // MNcash: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // MNcash: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // MNcash: track money supply and mint amount info
...
// MNcash: total coin age spent in transaction, in the unit of coin-days.
...
// MNcash: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // MNcash: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // MNcash: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // MNcash: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // MNcash: compute stake modifier
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if(pindexBest != NULL && pindexBest->nHeight > 1)
        nCoinbaseMaturity = 10; //coinbase maturity change to 10 blocks
    // Size limits
    ...
    if (IsProofOfStake())
    {
        ...
        // MNcash: check proof-of-stake block signature
...
// ----------- instantX transaction scanning -----------
	...
	CAmount masternodePaymentAmount = 0;
        if(IsProofOfStake())
        {
            masternodePaymentAmount = GetMasternodePayment(pindex->nHeight+1, vtx[1].GetValueOut());
	}
        else if(IsProofOfWork())
        {
            masternodePaymentAmount = GetMasternodePayment(pindex->nHeight+1, vtx[0].GetValueOut());
        }
...
bool CBlock::AcceptBlock()
{
    ...
    // MNcash: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // MNcash: ask for pending sync-checkpoint if any
	...
	if (pfrom) {
		...
		// Ask this guy to fill in what we're missing
	        ...
	        // MNcash: getblocks may not obtain the ancestor block rejected
    ...
    // MNcash: if responsible for sync-checkpoint send it
...
// mncash: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        strMiscWarning = strMessage;
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "MNcash", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Time is money, and money is now MNcash";
        CTransaction txNew;
        txNew.nTime = 1518134400;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1518134400;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 5012300;
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xafa0a0306e1f337bdea52741c9481ff0f20e670b38556d28077a2d1b78132b7c"));
        ...
        // MNcash: initialize synchronized checkpoint
...
unsigned char pchMessageStart[4] = { 0xa1, 0xb3, 0xc2, 0xd4 };
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // MNcash: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        if (pindex)    // only act when an explicit height is requested
        {
            if (pindex->nHeight > pfrom->nHighestHeightRequested)
            {
                ...
                if (pindex->nHeight < (pfrom->nHighestHeightRequested - 500))   // peer has gone backwards in height when requesting blocks
                {
                    ...
                    printf("peer %s: leap backwards in height request, from max %d to current %d. penalty=%d/100\n", pfrom->addr.ToString().c_str(), pfrom->nHighestHeightRequested, pindex->nHeight, pfrom->nHeightBackwards);
                    pfrom->nHeightBackwardsLast = GetTime();
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // MNcash: tell downloading node about the latest block if it's
    ...
    else
    {
        ...
        ProcessMessageAnonsend(pfrom, strCommand, vRecv);

...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "MNcash " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
static const char *strDNSSeed[][2] = {
    {"seed.mncash.org", "seed.mncash.org"},
    {"seed1.mncash.org", "seed1.mncash.org"},
    {"seed2.mncash.org", "seed2.mncash.org"},
    {"seed3.mncash.org", "seed3.mncash.org"},
};

void ThreadDNSAddressSeed(void* parg)
{
    // Make this thread recognisable as the DNS seeding thread
    RenameThread("MNcash-dnsseed");
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. MNcash is probably already running."), addrBind.ToString().c_str());
        else
...
void RelayAnonSendIn(const std::vector<CTxIn>& in, const int64_t& nAmount, const CTransaction& txCollateral, const std::vector<CTxOut>& out)
{
    ...
    BOOST_FOREACH(CNode* pnode, vNodes)
    {
        if((CNetAddr)anonSendPool.submittedToMasternode != (CNetAddr)pnode->addr) continue;
        printf("RelayAnonSendIn - found master, relaying message - %s \n", pnode->addr.ToString().c_str());

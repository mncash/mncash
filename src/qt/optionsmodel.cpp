...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeMNcashAmount"))
	 settings.setValue("nAnonymizeMNcashAmount", 1000);

    nAnonsendRounds = settings.value("nAnonsendRounds").toLongLong();
    nAnonymizeMNcashAmount = settings.value("nAnonymizeMNcashAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeMNcashAmount"))
  	SoftSetArg("-anonymizeMNcashamount", settings.value("nAnonymizeMNcashAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeMNcashAmount:
            return QVariant(nAnonymizeMNcashAmount);
        default:
            return QVariant();
        }
    }
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
	...
	case AnonsendRounds:
	     ...
	     break;
	case AnonymizeMNcashAmount:
	     nAnonymizeMNcashAmount = value.toInt();
	     settings.setValue("nAnonymizeMNcashAmount", nAnonymizeMNcashAmount);
	     emit anonymizeMNcashAmountChanged(nAnonymizeMNcashAmount);
	     break;
        default:
            break;
        }

...
static void NotifyMNcashNodeUpdated(MasternodeManager *page, CMNcashNodeConfig nodeConfig)
{
    ...
    QMetaObject::invokeMethod(page, "updateMNcashNode", Qt::QueuedConnection,
                              Q_ARG(QString, alias),
                              Q_ARG(QString, addr),
...
void MasternodeManager::subscribeToCoreSignals()
{
    ...
    uiInterface.NotifyMNcashNodeChanged.connect(boost::bind(&NotifyMNcashNodeUpdated, this, _1));
}

void MasternodeManager::unsubscribeFromCoreSignals()
{
    // Disconnect signals from core
    uiInterface.NotifyMNcashNodeChanged.disconnect(boost::bind(&NotifyMNcashNodeUpdated, this, _1));
}
...
void MasternodeManager::updateMNcashNode(QString alias, QString addr, QString privkey, QString collateral)
{
    LOCK(cs_mncash);
    bool bFound = false;
    int nodeRow = 0;
...
void MasternodeManager::updateNodeList()
{
    ...
    {
        LOCK(cs_mncash);
        BOOST_FOREACH(PAIRTYPE(std::string, CMNcashNodeConfig) mncash, pwalletMain->mapMyMNcashNodes)
        {
            updateMNcashNode(QString::fromStdString(mncash.second.sAlias), QString::fromStdString(mncash.second.sAddress), QString::fromStdString(mncash.second.sMasternodePrivKey), QString::fromStdString(mncash.second.sCollateralAddress));
        }
    }
}
...
{
    AddEditMNcashNode* aenode = new AddEditMNcashNode();
    aenode->exec();
}
...
void MasternodeManager::on_getConfigButton_clicked()
{
    ...
    int r = index.row();
    std::string sAddress = ui->tableWidget_2->item(r, 1)->text().toStdString();
    CMNcashNodeConfig c = pwalletMain->mapMyMNcashNodes[sAddress];
    std::string sPrivKey = c.sMasternodePrivKey;
    MNcashNodeConfigDialog* d = new MNcashNodeConfigDialog(this, QString::fromStdString(sAddress), QString::fromStdString(sPrivKey));
    d->exec();
}
...
void MasternodeManager::on_removeButton_clicked()
{
    ...
    confirm = QMessageBox::question(this, "Delete MN-MNcash Node?", "Are you sure you want to delete this mn-mncash node configuration?", QMessageBox::Yes|QMessageBox::No);
    ...
    if(confirm == QMessageBox::Yes)
    {
        ...
        pwalletMain->mapMyMNcashNodes.erase(sAddress);
        walletdb.EraseMNcashNodeConfig(c.sAddress);
...
void MasternodeManager::on_startButton_clicked()
{
    ...
    QMessageBox msg;
    if(result)
        msg.setText("MN-MNcash Node at " + QString::fromStdString(c.sAddress) + " started.");
    else
...
void MasternodeManager::on_stopButton_clicked()
{
    ...
    if(result)
    {
        msg.setText("MN-MNcash Node at " + QString::fromStdString(c.sAddress) + " stopped.");
    }
    else
    {
        msg.setText("Error: " + QString::fromStdString(errorMessage));
...
void MasternodeManager::on_startAllButton_clicked()
{
    ...
    BOOST_FOREACH(PAIRTYPE(std::string, CMNcashNodeConfig) mncash, pwalletMain->mapMyMNcashNodes)
    {
        CMNcashNodeConfig c = mncash.second;
	std::string errorMessage;
        bool result = activeMasternode.RegisterByPubKey(c.sAddress, c.sMasternodePrivKey, c.sCollateralAddress, errorMessage);
	if(result)
	{

...
void AnonsendConfig::clickBasic()
{
    ...
    QMessageBox::information(this, tr("Anonsend Configuration"),
        tr(
            "Anonsend was successfully set to basic (%1 and 2 rounds). You can change this at any time by opening MNcash's configuration screen."
        ).arg(strAmount)
    );
...
void AnonsendConfig::clickHigh()
{
    ...
    QMessageBox::information(this, tr("Anonsend Configuration"),
        tr(
            "Anonsend was successfully set to high (%1 and 8 rounds). You can change this at any time by opening MNcash's configuration screen."
        ).arg(strAmount)
    );
...
void AnonsendConfig::configure(bool enabled, int coins, int rounds) {
    ...
    settings.setValue("nAnonymizeMNcashAmount", coins);

    nAnonsendRounds = rounds;
    nAnonymizeMNcashAmount = coins;
}

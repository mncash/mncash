...
#include "ui_addeditmncashnode.h"
...
AddEditMNcashNode::AddEditMNcashNode(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::AddEditMNcashNode)
{
    ui->setupUi(this);

}

AddEditMNcashNode::~AddEditMNcashNode()
{
    delete ui;
}


void AddEditMNcashNode::on_okButton_clicked()
{
	...
	bool bForceNew = false;
        ...
        pwalletMain->mapMyMNcashNodes.insert(make_pair(c.sAddress, c));
	walletdb.WriteMNcashNodeConfig(c.sAddress, c);
        uiInterface.NotifyMNcashNodeChanged(c);

        accept();
    }
}

void AddEditMNcashNode::on_cancelButton_clicked()
{
    reject();
}

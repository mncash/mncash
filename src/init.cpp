...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("MNcash version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  MNcashd [options]                     " + "\n" +
                  "  MNcashd [options] <command> [params]  " + _("Send command to -server or MNcashd") + "\n" +
                  "  MNcashd [options] help                " + _("List commands") + "\n" +
                  "  MNcashd [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "mncash:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("MNcash"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("MNcash"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: MNcash.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: MNcashd.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 27415 or testnet: 47415)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 27414 or testnet: 47414)") + "\n" +
	...
	" -anonymizeMNcashamount=<n> " + _("Keep N MNcash anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  MNcash is probably already running."), strDataDir.c_str()));
    ...
    if (fDaemon)
        fprintf(stdout, "MNcash server starting\n");
    ...
    if (filesystem::exists(GetDataDir() / strWalletFileName))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("MNcash"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        }
    ...
    if (mapArgs.count("-reservebalance")) // MNcash: reserve balance amount
    {
    ...
    if (mapArgs.count("-checkpointkey")) // MNcash: checkpoint master priv key
    {
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of MNcash") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart MNcash to complete") << "\n";
...
fMasterNode = GetBoolArg("-masternode", false);
 ...
 nAnonymizeMNcashAmount = GetArg("-anonymizeMNcashamount", 0);
 if(nAnonymizeMNcashAmount > 999999) nAnonymizeMNcashAmount = 999999;
 if(nAnonymizeMNcashAmount < 2) nAnonymizeMNcashAmount = 2;
 ...
 if(nInstantXDepth < 0) nAnonymizeMNcashAmount = 0;
 } else {
 nInstantXDepth = 0;
 }
 ...
 printf("Anonymize MNcash Amount %d\n", nAnonymizeMNcashAmount);

 /* Denominations
 A note about convertability. Within Anonsend pools, each denomination
 is convertable to another.
 For example:
 1MNcash+1000 == (.1MNcash+100)*10
 10MNcash+10000 == (1MNcash+1000)*10
 */

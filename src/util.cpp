...
int nAnonymizeMNcashAmount = 500;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MNcash
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MNcash
    // Mac: ~/Library/Application Support/MNcash
    // Unix: ~/.MNcash
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MNcash";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "MNcash.conf"));
    if (!pathConfigFile.is_complete()) pathConfigFile = GetDataDir(false) / pathConfigFile;
    return pathConfigFile;
}
...
void createConf()
{
    ...
    + "\ntestnet=0"
    + "\naddnode=seed.mncash.org"
    + "\naddnode=seed1.mncash.org"
    + "\naddnode=seed2.mncash.org"
    + "\naddnode=seed3.mncash.org";
   pConf.close();
}
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "MNcashd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong MNcash will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("MNcash"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
